...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly date)
    {
        ...
        CreateTotalSpentSection(page, date, totalSpent);
        ...
        foreach (var expense in expenses)
        {
            ...
            row.Height = HEIGHT_ROW_TABLE_EXPENSES;
            ...
            if (!string.IsNullOrWhiteSpace(expense.Description))
            {
                ...
                descriptionRow.Height = HEIGHT_ROW_TABLE_EXPENSES;
    ...
    private static void CreateHeaderWithLogoAndName(string userName, Section page)
    {
        ...
        var logo = Path.Combine(dir!, "Logo", "logo.png");
    ...
    private static void CreateTotalSpentSection(Section page, DateOnly date, decimal totalSpent)
    {
        var parahraph = page.AddParagraph();
        parahraph.Format.SpaceBefore = 40;
        parahraph.Format.SpaceAfter = 40;

        var title = string.Format(ResourceReportGenerationMessages.TOTAL_SPENT_IN, date.ToString("Y"));
        parahraph.AddFormattedText(title, new Font { Name = FontHelper.RALEWAY_REGULAR, Size = 15 });

        parahraph.AddLineBreak();

        parahraph.AddFormattedText($"{totalSpent:F2} {CURRENCY_SYMBOL}", new Font { Name = FontHelper.WORKSANS_BLACK, Size = 50 });
    }
    ...
    {
        cell
           .AddParagraph(ResourceReportGenerationMessages.AMOUNT)
          .Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 14, Color = ColorsHelper.WHITE };
        cell.Shading.Color = ColorsHelper.RED_DARK;
        cell.VerticalAlignment = VerticalAlignment.Center;
    }

    private static void SetStyleBaseForExpenseInfo(Cell cell)
    {
    ...
    private static void AddAmountForExpense(Cell cell, decimal amount)
    {
        cell.AddParagraph($"-{amount:F2} {CURRENCY_SYMBOL}")
                .Format.Font = new Font { Name = FontHelper.WORKSANS_REGULAR, Size = 14, Color = ColorsHelper.BLACK };
